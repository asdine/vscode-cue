...
export async function ensureTools(log: vscode.OutputChannel): Promise<boolean> {
    ...
    try {
        ...
        try {
            ...
            log.show(true);
            log.appendLine(
                `cueimports is installed but not in the path. Please add ${toolsPath} to your PATH.`
            );
            log.appendLine(
                "You can also change the install location by updating the cue.toolsPath setting."
            );
            return false;
        } catch (err) {
            log.show(true);
...
async function installLatestToolsVersion(
...
async function downloadAndInstall(
    ...
    try {
        ...
        if (platform === "darwin") {
            asset = assets.find((a) =>
                a.name.toLowerCase().includes("macos_universal")
            );
        } else {
            asset = assets.find(
                (a) =>
        ...
        if (!asset) {
            log.appendLine(
                `No cueimports build found for platform ${platform} and arch ${arch}`
            );
            log.appendLine(
                `You can build cueimports from source at https://github.com/asdine/cueimports`
            );
            return;
        }
        const { browser_download_url: browserDownloadUrl } = asset;
        log.appendLine(`Downloading cueimports from ${browserDownloadUrl} ...`);

        // download and untar the asset
        const q = https.get(browserDownloadUrl, async (res) => {
            if (res.statusCode !== 200) {
                log.appendLine(
                    `Failed to download cueimports: ${res.statusMessage} ${res.statusCode}`
                );
                return;
            }
            ...
            await withTempDir(async (dir: string) => {
                await new Promise<void>((resolve) => {
                    const finish = async () => {
                        ...
                        log.appendLine(
                            `cueimports downloaded successfully to ${dest}`
                        );
                        log.appendLine(
                            `Please make sure ${dest} is in your PATH.`
                        );
                        log.appendLine(
                            "You can also change the install location by setting the cue.toolsPath setting."
                        );
                        log.appendLine("");
...
async function shouldUpdate(
    ...
    try {
        ...
        if (current === "development") {
            log.appendLine(
                "cueimports is installed from source. Skipping update."
            );
            return false;
        }
        ...
        if ("v" + current !== tag) {
            log.appendLine(
                `cueimports is out of date. Current version is ${current}, latest is ${tag}.`
            );
            return true;
        }
        ...
        if (err.message.includes("flag provided but not defined: -version")) {
            log.appendLine(
                `cueimports is out of date. Current version is v0.1.0, latest is ${tag}.`
            );
            return true;
        }

        log.appendLine(
            `Couldn't check cueimports version: ${e}. Skipping error and updating cueimports.`
        );
        return true;
    }
}
